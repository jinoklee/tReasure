  ...
  hg19.fa <- "E:/WholeGenomeFasta/hg19/hg19.fa"
  hg38.fa <- "E:/WholeGenomeFasta/hg38/hg38.fa"
  mm10.fa <- "E:/WholeGenomeFasta/mm10/mm10.fa"
  
  hg19.gtf <- "E:/gtf/hg19-tRNAs.gtf"
  hg38.gtf <- "E:/gtf/hg38-tRNAs.gtf"
  mm10.gtf <- "E:/gtf/mm10-tRNAs.gtf"
  
  hg19.annot <- "E:/data/duplicate_tRNA_hg19.txt"
  hg38.annot <- "E:/data/duplicate_tRNA_hg38.txt"
  mm10.annot <- "E:/data/duplicate_tRNA_mm10.txt"
  intro <- "E:/image/intro.png"
  ...
  volcano <- function(width, height, res){
    ...
    up <- outt$FDR < svalue(widget_list$pval) & outt$logFC > svalue(widget_list$FC)
    down <- outt$FDR < svalue(widget_list$pval) & outt$logFC < -svalue(widget_list$FC)
    
    png("volcanoplot_trna%02d.png", height=height, width=width, res=res)
    plot(outt$logFC, -log10(out$FDR), xlab="log2 Fold Change",ylab = "adjusted p_value", pch=19, cex=1.3,col="grey89")
    points(outt$logFC[up], -log10(outt$FDR)[up], pch=19, cex=1.3,col="#EF8A62")
    points(outt$logFC[down], -log10(outt$FDR)[down], pch=19, cex=1.3,col= "#67A9CF")
    legend("bottomright", c("Up_DEtRNA", "Non-DEtRNA", "Down_DEtRNA"), col=c("#EF8A62", "black", "#67A9CF"), pch=19, cex=1)
    abline(v=c(-(svalue(widget_list$FC)),svalue(widget_list$FC)), col="black",lty=2)
    abline(h=-log10(svalue(widget_list$pval)), col="black", lty=2)
    graphics.off()
  }
  ...
  barplot <- function(width, height, res){
    ...
    if(svalue(widget_list$fdr_s) == "Benjamini-Hochberg"){
      dw <- filter(out, logFC < - svalue(widget_list$FC), out$Benjamini < svalue(widget_list$pval))
      up <- filter(out, logFC > svalue(widget_list$FC), out$Benjamini < svalue(widget_list$pval))
    }else if(svalue(widget_list$fdr_s) =="Bonferroni"){
      dw <- filter(out, logFC < -svalue(widget_list$FC), out$Bonferroni< svalue(widget_list$pval))
      up <- filter(out, logFC > svalue(widget_list$FC), out$Bonferroni < svalue(widget_list$pval))
    }else{
      dw <- filter(out, logFC < -svalue(widget_list$FC), out$FDR < svalue(widget_list$pval))
      up <- filter(out, logFC > svalue(widget_list$FC), out$FDR < svalue(widget_list$pval))
    }
    ...
    png("barplot_isodecoder%02d.png", height=height, width=width, res=res)
  ...
  pyramid <- function(width, height, res){
    ...
    png("pyramid_isoaccepter%02d.png", width=width, height=height,  res=res)
  ...
  # gr1 : Sample List----------------
        ...
        addHandlerChanged(make_button, handler = function(h,...){
          ...
          if(identical(b,character(0))){
            ...
            if(length(fq_list)%%2 == 0){len = length(fq_list)/2}else{len=round(length(fq_list)/2+1)}
        ...
        addHandlerChanged(sel_button, handler = function(h,...){
          ...
          #tbl <- gdf(sample2, container = ggr1)
          tbl[] <- sample2
  ...
  # gr2 : Preprocessing----------------
        ...
        addHandlerClicked(trim_button, handler = function(h,...){
          if(file.exists("sample.txt") == FALSE & file.exists("*.fastq") == FALSE){
            ...
            dir <- svalue(fq_dir)
            sam <- file.path(svalue(fq_dir),"sample.txt")
            sam_trim <- sub(".txt", "_trim.txt", sam)
            sFile1 <- read.delim(sam, header = TRUE, as.is = TRUE)
  ...
  # gr3 : Analysis-------------------
        ...
        # handler --------------
        addHandlerChanged(selfq_button, handler = function(h,...){setwd(svalue(selfq_button))})
        ...
        addHandlerClicked(anl_button,handler = function(h, ...){
          insert(st, " ", do.newline = TRUE)
          insert(st,"****************", do.newline = TRUE ) 
          insert(st,"[ START alignment ]", do.newline = TRUE ) 
          b <- svalue(fq_dir)
          ...
          if(
            ...
            sFile <- file.path(dir, "sample_trim.txt")
                  ...
                  algin_status <- function(){
                    for(i in sFile2$SampleName){
                      ...
                      repeat{
                        Sys.sleep(1)
                        insert(st, ".", do.newline = FALSE)
                        list <- list.files(pattern=".bam$", full.names = TRUE)
            ...
            bFile <- list.files(dir, pattern = ".bam$", full.names = T)
                  ...
                  readc <- future(readcount())
                
                  insert(st, " ", do.newline = TRUE)
                  insert(st,"****************", do.newline = TRUE ) 
                  insert(st, "[ Start read counts ]", do.newline = TRUE)
                  
                  for(i in 1:1){
                    repeat{
                      Sys.sleep(1)
                      insert(st, ".", do.newline = FALSE)
                      if(file.exists("readcount_trna.txt")) break
                    }
                    insert(st, " ", do.newline = TRUE)
                    insert(st, paste("complete   :  Readcount.txt "), do.newline=TRUE)
                  }
                  ...
                  insert(st, " ", do.newline = TRUE)  
                  insert(st,"[ DONE alignment! ] Click! Next tab of Preview ", do.newline = TRUE )
  ...
  # gr4 : Preview--------------------
        ...
        addHandlerChanged(Pre_button, handler = function(h, ...){
          ...
          if(identical(b,character(0))){invisible()}else{setwd(svalue(selrc_button))}
          ...
          filter_gene <- function(count){
            ...
            #isexpr <- rowSums(cpm(x) > svalue(cfilv)) >= svalue(sfilv)*nrow(sFile)/100
            x <- x[isexpr,]
            return(x)
          }
          ...
          cols[cols=="test"] ="red"
          png( 'plotMDS.png', width=650, height=500,  res=80)
          plotMDS(y,col=cols)
          graphics.off()
          gimage("plotMDS.png", container = Pre_MDS)
          
          insert(st, " ", do.newline = TRUE)  
          insert(st,"[ Saving MDS_plot is done! as png format ] Click! Next tab of DEtRNA list ", do.newline = TRUE )
  ...
  # gr5 : DEtRNA list----------------
        ...
        addHandlerClicked(statedgeR_button, handler = function(h, ...){
          ...
          insert(st, " ", do.newline = TRUE)  
          insert(st," Saving the result files of satistical analysis is done!. Adjust the threshold value.", do.newline = TRUE )
        })
        ...
        # Plot option --------------------
        DE_button <- gbutton(" Analysis for DEtRNA ", container = ggr.53)
        
        # Plot handler --------------------
        addHandlerClicked(DE_button, handler = function(h, ...){
          insert(st," ", do.newline = TRUE ) 
          insert(st,"[ Plots ]", do.newline = TRUE )
          # t_out <- read.delim("./stat/stat_trna_list.txt")
          # c_out <- read.delim("./stat/stat_isodecoder_list.txt")
          # a_out <- read.delim("./stat/stat_isoacceptor_list.txt")
          
          # DEtRNA <- DE_function(t_out)
          # DEcodon <- DE_function(c_out)
          # DEaa <- DE_function(a_out)
          ...
          volcano(650, 460, 80)
          v_plot <- gimage("volcanoplot_trna01.png", container = Plot_trna)
          
          barplot(850, 460,80)
          b_plot <- gimage("barplot_isodecoder01.png", container = Plot_codon)
          
          pyramid(650, 460,80)
          p_plot <- gimage("pyramid_isoaccepter01.png", container = Plot_aa)
          insert(st, " ", do.newline = TRUE)  
          insert(st,"Saving the plots of satistical analysis is done!. Click! Next tab of Plot", do.newline = TRUE )
        })
  ...
  # gr6 : Plots----------------------
        ...
        tsave <- gbutton(" Save ", container = tmp.61,expand=TRUE, fill="x", handler= function(h, ...){
          insert(st," ", do.newline = TRUE ) 
          insert(st,"Saving the re-sized plots is done!", do.newline = TRUE )
